...
local files_Mortal = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
...
local keko_add_sudo = redis:get('Mortal:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('Mortal:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('Mortal:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creatorbasic = database:sismember('Mortal:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local admin = database:sismember('Mortal:'..bot_id..'admins:', user_id)
...
local var = false
local creatorbasic = database:sismember('Mortal:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local creator = database:sismember('Mortal:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('Mortal:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('Mortal:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('Mortal:'..bot_id..'admins:', user_id)  
local owner = database:sismember('Mortal:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('Mortal:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = database:sismember('Mortal:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local vip = database:sismember('Mortal:'..bot_id..'vipgp:'..chat_id, user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('Mortal:'..bot_id..'SET:BAN'..chat_id, user_id)  
local creator = database:sismember('Mortal:'..bot_id..'creator:'..chat_id, user_id)  
if mod then var = true end
if owner then var = true end
if creator then var = true end
...
local var = false
local banned = database:sismember('Mortal:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('Mortal:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('Mortal:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('Mortal:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
{
{text = ''..(channel_ts or "Mortal TEAM")..'', url=''..(channel_user_ts or 't.me/Mortal_files')..''},
},
}
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'Mortal')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'Mortal')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
local res = https.request('https://Mortal.ga/Mortal.php?id='..msg.sender_user_id_..'')
...
database:del( 'Mortal:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
local files_Mortal = database:smembers("files"..bot_id)
local keko = io.popen('cd files_Mortal && ls'):read("*all")
local files_Mortal2 = ''
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'Mortal_files')..')\n\n☑┇تم رفعه مطور'
else
texts = '✖┇خطاء'
end
...
local hash =  'Mortal:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'Mortal_files')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'Mortal:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'Mortal_files')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
os.execute('cd libs && wget https://raw.githubusercontent.com/MortalTEAM/Mortal/master/libs/utils.lua')
os.execute('rm -rf Mortal.lua')
os.execute('wget https://raw.githubusercontent.com/MortalTEAM/Mortal/master/Mortal.lua')
os.exit()
return false
end
if text == "تفعيل"  then
...
database:set( 'Mortal:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
end
...
database:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Mortal"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'Mortal:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
database:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
function TSby(extra,result,success)
info = '👁‍🗨※بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'Mortal_files')..')\n'
if not database:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
...
database:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Mortal"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local tsX_o = database:get("lock_tag:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
database:set("lock_tag:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = database:get("lock_reple:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
database:set("lock_reple:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
database:set("lock_sarha:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكلايش ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكلايش ")
database:set("lock_word:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") and (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = database:get("lock_edit:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
database:set("lock_edit:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
database:set("lock_lllll:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المتحركه ")
else
tsX000("lock",msg,"🔐┇ تم قفل المتحركه ")
database:set("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
database:set("lock_files:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الماركدون ")
else
tsX000("lock",msg,"🔐┇ تم قفل الماركدون ")
database:set("lock_mark:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الصور ")
else
tsX000("lock",msg,"🔐┇ تم قفل الصور ")
database:set("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملصقات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملصقات ")
database:set("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الفيديو ")
else
tsX000("lock",msg,"🔐┇ تم قفل الفيديو ")
database:set("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
database:set("lock_inline:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
database:set("lock_chat:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
database:set("lock_fwd:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
database:set("lock_pin:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الاغاني ")
else
tsX000("lock",msg,"🔐┇ تم قفل الاغاني ")
database:set("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الصوت ")
else
tsX000("lock",msg,"🔐┇ تم قفل الصوت ")
database:set("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
database:set("lock_contact:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
database:set("lock_ar:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
database:set("lock_en:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الميديا ")
else
tsX000("lock",msg,"🔐┇ تم قفل الميديا ")
database:set("lock_media:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الروابط ")
else
tsX000("lock",msg,"🔐┇ تم قفل الروابط ")
database:set("lock_link:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
database:set("lock_username:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الاشعارات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الاشعارات ")
database:set("lock_new:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
database:set("lock_botAndBan:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
database:set("lock_bot:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
database:set("lock_join:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_tag:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_sarha:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_word:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_edit:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_lllll:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_files:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_mark:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_contact:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_fwd:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_link:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_username:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_botAndBan:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_new:Mortal"..msg.chat_id_..bot_id,"ok")
...
database:del("lock_reple:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:Mortal"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_inline:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:Mortal"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End Mortal unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:Mortal"..msg.chat_id_..bot_id) then 
local Mortal_wr = (database:get("Mortal:not:word:"..bot_id..msg.chat_id_) or 100)
...
if database:get("lock_link:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.reply_to_message_id_ ~= 0 then
if database:get("lock_reple:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if  msg.content_.ID == "MessageChatJoinByLink" then
if database:get("lock_join:Mortal"..msg.chat_id_..bot_id) then
...
if database:get("lock_bot:Mortal"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:Mortal"..msg.chat_id_..bot_id) then
...
if database:get("lock_fwd:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:Mortal"..msg.chat_id_..bot_id) then
...
database:set("lock_link:Mortal"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:Mortal"..msg.chat_id_..bot_id,"ok")
database:set('Mortal:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if text == 'مغادره' then
local lave = database:get('Mortal:'..bot_id..'leave:groups')
print(lave)
if not database:get('Mortal:'..bot_id..'leave:groups') then
...
database:del('Mortal:'..bot_id..'leave:groups')
end
...
database:set('Mortal:'..bot_id..'leave:groups',true)
end
if text == ("مسح الاساسين") then
text = '☑┇تم مسح قائمه المنشئين الاساسين'
database:del('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_)
...
function promote_by_reply(extra, result, success)
database:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'Mortal')..')\n┇ 🔖┇تم رفعة منشئ اساسي  في البوت  '
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
function promote_by_reply(extra, result, success)
database:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
local group = database:smembers('Mortal:'..bot_id..'groups') 
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]
},function(arg,data) end,nil)
end
function fwwdmsg(taha,storm,sorc)  
local list = database:smembers('Mortal:'..bot_id..'groups') 
...
local gps = database:scard('Mortal:'..bot_id.."groups")  
...
local pv = database:smembers('Mortal:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
function fwwdmsg(taha,storm,sorc)  
local list = database:smembers('Mortal:'..bot_id.."userss")
for k,v in pairs(list) do  
tdcli_function ({
ID = "ForwardMessages",
chat_id_ = v,
...
local gps = database:scard('Mortal:'..bot_id.."userss")  
...
if (not database:get('Mortal:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)\n ☑┇ ارسل (الغاء) للغاء الاذاعه', 1, 'md')
database:set("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "Mortal"
else
...
if (not database:get('Mortal:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
database:setex('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)\n ☑┇ ارسل (الغاء) للغاء الاذاعه', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
...
database:srem('Mortal:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
database:srem('Mortal:'..bot_id..'pro:groups',group[i]) 
database:srem( 'Mortal:'..bot_id.."groups",group[i]) 
...
local gps = database:scard('Mortal:'..bot_id.."groups") or 0
local user = database:scard('Mortal:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('Mortal:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('Mortal:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('Mortal:'..bot_id.."enable:"..gps[i],true)
database:set( 'Mortal:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('Mortal:'..bot_id.."enable:"..gps[i])
database:del( 'Mortal:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('Mortal:'..bot_id.."groups")
end
end 
		if text == ("روابط الكروبات") then
local gpss = database:smembers('Mortal:'..bot_id.."groups") or 0
...
if database:get('Mortal:'..bot_id.."group:link"..gpss[i]) then 
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(database:get('Mortal:'..bot_id.."group:link"..gpss[i])).."\n"
else
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link.result or ' لا توجد روابط ').."\n"
end
end
local f = io.open('Mortal_Links.txt', 'w')
f:write(text233)
f:close()
local Mortale = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Mortale .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'Mortal_Links.txt' .. '"'
io.popen(curl) 
end
		----------------
...
for i=1, #gpss do
local link = database:get('Mortal:'..bot_id.."group:link"..gpss[i])
...
local time = a[2] * day
database:setex( 'Mortal:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'Mortal:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'Mortal:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'Mortal:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'Mortal:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
...
database:setex( 'Mortal:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'Mortal:'..bot_id.."charge:"..txt[2],true)
...
local photo = database:get('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'Mortal:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'Mortal:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'Mortal:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Mortal:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('Mortal:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'Mortal:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Mortal:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('Mortal:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('Mortal:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
database:set("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id,Mortal_edis[1])
database:sadd('Mortal:'..bot_id..'Mortalnwe:'..msg.chat_id_,Mortal_edis[1])
...
database:set("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("Mortal:edit:text:su:newamr2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = database:get("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
database:del("Mortal:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
database:srem('Mortal:'..bot_id..'Mortalnwe:'..msg.chat_id_,klma)
...
local list = database:smembers('Mortal:'..bot_id..'Mortalnwe:'..msg.chat_id_)
if #list == 0 then
...
for k,v in pairs(list) do
var = database:get("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
local list = database:smembers('Mortal:'..bot_id..'Mortalnwe:'..msg.chat_id_)
for k,v in pairs(list) do
local klma = database:get("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if klma ~= nil then
database:del("Mortal:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Mortal:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
end
end
database:del('Mortal:'..bot_id..'Mortalnwe:'..msg.chat_id_)
...
if result.id_ then
database:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'Mortal')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local list = database:smembers('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
...
for k,v in pairs(list) do
local user_info = database:hgetall('Mortal:'..bot_id..'user:'..v)
...
if text:match("^رفع ادمن$")  and (is_owner(msg) or is_creatorbasic(msg)) and msg.reply_to_message_id_ then
 if not is_creator(msg) and database:get("Mortal:lock:promote"..bot_id..msg.chat_id_) then 
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
database:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1, "⚙️※ عذرا خاصية الرفع تم تعطيلها ❌ ", 1, 'md')
return false end
database:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'Mortal')
return "Mortal"
end
...
database:set("Mortal:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("Mortal:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and not is_setban(msg) and database:get("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
...
if not is_creator(msg) and database:get("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
...
database:sadd('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'Mortal')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
database:sadd('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:srem('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:del('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_)
...
if result.id_ then
database:del('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'Mortal')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('Mortal:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
if result.id_ then
if database:sismember('Mortal:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('Mortal:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('Mortal:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('Mortal:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
database:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
database:srem('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
if result.id_ then
local hash =  'Mortal:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'Mortal')..')\n🚫┇تم حظره من المجموعات البوت'
database:sadd(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'Mortal:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'Mortal')..')\n��┇تم الغاء حظره من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'Mortal:'..bot_id..'gmuted:'
if result.id_ then
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apid[2] or 'Mortal_files')..')\n🚫┇تم الغاء كتمه من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
database:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'Mortal')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('Mortal:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apki[2] or 'Mortal_files')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
database:srem('Mortal:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
database:srem('Mortal:'..bot_id..'dev', result.sender_user_id_)
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
t = '( المطورين - المنشئين الاساسين - المنشئين - المدراء - الادمنيه - المميزين )'
elseif redis:sismember('Mortal:'..bot_id..'dev',msg.sender_user_id_) then
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
...
elseif database:sismember('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_) then
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
t = '( المنشئين - المدراء - الادمنيه - المميزين )'
elseif database:sismember('Mortal:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) then
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
t = '( المدراء - الادمنيه - المميزين )'
elseif database:sismember('Mortal:'..bot_id..'owners:'..msg.chat_id_, msg.sender_user_id_) then
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
t = '( الادمنيه - المميزين )'
end
...
if result.id_ then
database:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("Mortal:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("Mortal:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
database:set("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_,"MortalE")
...
database:del("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
local Mortal_098 = 0
if database:get("Mortal:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("Mortal:get:mod:"..bot_id..msg.chat_id_)
...
database:set("Mortal:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
t = database:get("Mortal:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = database:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Mortal:all_if:"..database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ  الكروب '
elseif (database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Mortal:all_if:"..database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("Mortal:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = database:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = database:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
...
if not database:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
Mortal_oop = database:get("Mortal:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت'
elseif is_creatorbasic(msg) then
Mortal_oop = database:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي'
elseif (database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("Mortal:all_if:"..database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
Mortal_oop = database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
Mortal_oop = database:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب'
elseif (database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("Mortal:all_if:"..database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
Mortal_oop = database:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
Mortal_oop = database:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
Mortal_oop = database:get("Mortal:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
Mortal_oop = database:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
Mortal_oop = database:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
if result.id_ then
if not database:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
database:set('Mortal:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,text)
...
database:set('Mortal:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("Mortal:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('Mortal:'..bot_id.."group:link"..msg.chat_id_) and database:get('Mortal:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..database:get('Mortal:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
...
database:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
database:set('Mortal:'..bot_id.."welcome"..msg.chat_id_,true)
end
...
database:del('Mortal:'..bot_id.."welcome"..msg.chat_id_)
end
...
database:set('Mortal:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
n = (n + 1)
local user_info = database:hgetall('Mortal:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
function mute_by_reply(extra, result, success)
database:del('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
add = (tonumber(database:get('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = database:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📮┇ سحكاتك   *("..edit..")*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
database:del('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
...
database:set("Mortal:not:word:"..bot_id..msg.chat_id_,Mortal_0[2])
end
...
database:del('Mortal:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('Mortal:'..bot_id..'creator:'..msg.chat_id_)
...
database:del('Mortal:'..bot_id..'mods:'..msg.chat_id_)
...
database:del('Mortal:'..bot_id..'vipgp:'..msg.chat_id_)
...
database:set("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_,"Mortal")
...
database:del("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
database:set("Mortal:lock:promote"..bot_id..msg.chat_id_,"Mortal")
...
database:del("Mortal:lock:promote"..bot_id..msg.chat_id_)
...
if database:get("lock_media:Mortal"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('Mortal:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('Mortal:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'Mortal:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'Mortal:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:Mortal"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:Mortal"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:Mortal"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:Mortal"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('Mortal:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('Mortal:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('Mortal:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('Mortal:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'Mortal:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'Mortal:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local tsX_o = database:get("group_edit:Mortal"..msg.chat_id_..bot_id)
if not tsX_o then
database:set("group_edit:Mortal"..msg.chat_id_..bot_id,true)
...
--------------------------------
if (text and text == (redis:get('Mortal:'..bot_id..'name_bot') or 'تشاكي'))then
name_bot = (redis:get('Mortal:'..bot_id..'name_bot') or 'تشاكي')
local namebot = {
...
local list = database:smembers('Mortal:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
database:del('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Mortal:'..bot_id.."add:reply:rd"..v..msg.chat_id_)  
database:srem('Mortal:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
...
for k,v in pairs(list) do  
local taha = database:get('Mortal:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
database:set('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
database:set('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set('Mortal:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd('Mortal:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
...
database:set('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test and test == 'reppp' then   
send(msg.chat_id_, msg.id_, 1, "📮┇ تم الغاء منعها من قائمه المنع \n", 1, 'md')  
database:del('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Mortal:'..bot_id.."add:reply:rd"..text..msg.chat_id_)  
database:srem('Mortal:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del('Mortal:'..bot_id.."add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get('Mortal:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set('Mortal:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
database:del('Mortal:'..bot_id.."add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
if text and (not is_mod(msg) or not is_creatorbasic(msg)) then  
local taha = database:get('Mortal:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
if taha then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/Mortal)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
...
redis:del('Mortal:'..bot_id..'keko'..text..''..msg.chat_id_..'')
redis:del('Mortal:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('Mortal:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('Mortal:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('Mortal:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('Mortal:'..bot_id..':file:'..text..''..msg.chat_id_..'')
...
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'', 'msg')
return false end
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'', 're')
redis:set('Mortal:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ' , 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
redis:del('Mortal:'..bot_id..':sticker:'..text)
redis:del('Mortal:'..bot_id..':voice:'..text)
redis:del('Mortal:'..bot_id..':video:'..text)
redis:del('Mortal:'..bot_id..':gif:'..text)
redis:del('Mortal:'..bot_id..':file:'..text)
redis:del('Mortal:'..bot_id..'keko'..text)
redis:srem('Mortal:'..bot_id..'kekoresudo', text)
end
end
...
redis:sadd('Mortal:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
end

if text and not database:get('Mortal:'..bot_id..'repowner:mute'..msg.chat_id_) then
local keko = redis:get('Mortal:'..bot_id..'keko'..text..''..msg.chat_id_..'')
if keko then 
function keko_Mortal_re(t1,t2)
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي '
...
t = database:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("Mortal:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = database:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز'
else
t = database:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
getUser(msg.sender_user_id_, keko_Mortal_re)
end
if (redis:get('Mortal:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
end
if (redis:get('Mortal:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
end
if (redis:get('Mortal:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':video:'..text..''..msg.chat_id_..''))
end
if (redis:get('Mortal:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
end
if (redis:get('Mortal:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end  
end

if not database:get('Mortal:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local keko = redis:get('Mortal:'..bot_id..'keko'..text..'')
if keko then 
function keko_Mortal_re(t1,t2)
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
...
if (redis:get('Mortal:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':sticker:'..text))
end
if (redis:get('Mortal:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':voice:'..text))
end
if (redis:get('Mortal:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':video:'..text))
end
if (redis:get('Mortal:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':gif:'..text))
end
if (redis:get('Mortal:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Mortal:'..bot_id..':file:'..text))
end  
end
...
for k,v in pairs(list) do
redis:del('Mortal:'..bot_id..'kekore'..msg.chat_id_..'', text)
redis:del('Mortal:'..bot_id..'keko'..v..''..msg.chat_id_)
redis:del('Mortal:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..'keko'..v..msg.chat_id_)
end
...
local list = redis:smembers('Mortal:'..bot_id..'kekoresudo')
for k,v in pairs(list) do
redis:del('Mortal:'..bot_id..'kekoresudo', text)
redis:del('Mortal:'..bot_id..'keko'..v..'')
redis:del('Mortal:'..bot_id..':sticker:'..v)
redis:del('Mortal:'..bot_id..':voice:'..v)
redis:del('Mortal:'..bot_id..':video:'..v)
redis:del('Mortal:'..bot_id..':gif:'..v)
redis:del('Mortal:'..bot_id..':file:'..v)
redis:del('Mortal:'..bot_id..'keko'..v)
end
...
database:set('Mortal:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('Mortal:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:get("group_edit:Mortal"..msg.chat_id_..bot_id) then
...
database:set('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:get("group_edit:Mortal"..msg.chat_id_..bot_id) and  (is_creator(msg) or is_creatorbasic(msg)) then 
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
...
database:sadd('Mortal:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('Mortal:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del("Mortal:all_if:"..text..bot_id..msg.chat_id_)
...
database:set("Mortal:name_sudo"..bot_id..msg.chat_id_,ac[1])
...
database:set("Mortal:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:set("Mortal:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text:match("^تغير رد العضو فقط (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
database:set("Mortal:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه" and (is_creator(msg) or is_creatorbasic(msg))) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Mortal"
end
if text and database:get("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Mortal"
end
if (text and database:get("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("Mortal:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("Mortal:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function keko33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'Mortal')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("Mortal:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'Mortal')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("Mortal:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
database:del("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'Mortal')..')\n🔰┇تم تنزيله من '..apmd[1]
database:del("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"🔰┇تم تنزيله من "..apmd[1])
database:del("Mortal:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('Mortal:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('Mortal:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('Mortal:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("Mortal"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("Mortal"..bot_id..":sudo:gr")
end
if (text:match("^ايدي$") or text:match("^id$") ) and msg.reply_to_message_id_ == 0 then
if not database:sismember('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
      database:sadd('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
...
local tahna = (database:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
...
if not database:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("Mortal:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه 😐",
"وفالله 😔💘",
...
local Mortal_new_text = database:get("Mortal:gr:id:text:"..bot_id..msg.chat_id_)
...
if text then
if database:sismember('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
if not database:get('Mortal:'..bot_id..'pin:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تفعيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل التثبيت', 1, 'md')
database:del('Mortal:'..bot_id..'pin:mute'..msg.chat_id_)
end
end
...
database:set('Mortal:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('Mortal:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
database:sadd('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
database:sadd('Mortal:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
database:sadd('Mortal:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
database:srem('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
database:srem('Mortal:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
database:srem('Mortal:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
...
database:del('Mortal:'..bot_id.."filteranimation"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع المتحركات')  
end
if text == 'مسح قائمه منع الصور' and is_owner(msg) then 
database:del('Mortal:'..bot_id.."filterphoto"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع الصور')  
end
if text == 'مسح قائمه منع الملصقات' and is_owner(msg) then 
database:del('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_)
sendtext(msg.chat_id_, msg.id_,'🔖┇ تم مسح قائمه منع الملصقات')  
end

---------------
if text == 'سمايلات' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Mortal:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','??','🥐','🍞','??','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','??','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..'klmos'..msg.chat_id_,name)
...
if text == ''..(database:get('Mortal:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Mortal:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    

end
database:set('Mortal:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
if text == 'الاسرع' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Mortal:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..'klmo'..msg.chat_id_,name)
...
if text == ''..(database:get('Mortal:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Mortal:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'l:id'..msg.chat_id_) then 
...
database:set('Mortal:'..bot_id..'l:id'..msg.chat_id_,true)
end 
if text == 'حزوره' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Mortal:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..'klmoa'..msg.chat_id_,name)
...
if text == ''..(database:get('Mortal:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Mortal:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Mortal:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
if text == 'المعاني' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Mortal:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..'means'..msg.chat_id_,name)
...
if text == ''..(database:get('Mortal:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Mortal:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Mortal:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
if text == 'العكس' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Mortal:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..'aks'..msg.chat_id_,name)
...
if text == ''..(database:get('Mortal:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Mortal:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('Mortal:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Mortal:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
				if database:get('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Mortal:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖¦ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸¦ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:incrby('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(database:get('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
database:del('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
if database:get('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Mortal:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮¦ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊¦ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:del('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
if text == database:get('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_) then -- // المختلف
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == database:get('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_) then -- // امثله
database:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == 'المختلف' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_,name)
name = string.gsub(name,'😸','😹😹😹😹😹😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠','💀💀💀💀💀💀💀☠💀💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼👻👻👻👻👻')
name = string.gsub(name,'🐇','🕊🕊🕊🕊🕊🐇🕊🕊🕊🕊')
...
if text == 'امثله' and database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
database:set('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
...
if database:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
database:set('Mortal:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
...
database:set('Mortal:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n📮¦ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠¦ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖¦ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n??*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
if text =='مجوهراتي' then 
if tonumber((database:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(database:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(database:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
database:set('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
taha = (database:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل مجوهره تساوي '..taha..' رساله', 'md')
end
end
if text == 'حذف مجوهراتي' then
database:del('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
if text == 'حذف رسائلي' then
database:del('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
...
database:set('Mortal:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:del('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
🌐┇<strong>Mortal TEAM</strong>

🦁┇<a href="https://telegram.me/Mortal">قناه السورس</a>
🦁┇<a href="https://t.me/Mortal_files">قناه ملفات السورس</a>

🔎┇<a href="https://t.me/MortalMake_BOT">صنع بوتات حمايه 🥇</a>
...
📮┇<a href="t.me/Mortalbbot">لـ تواصل </a> 
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
...
if data.username_ ~= false then
database:hset('Mortal:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
z_Mortal = TSCheckMsg(msg)
if z_Mortal and z_Mortal == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:Mortal"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("Mortal:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local Mortal_edit_text = database:get("Mortal:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
...
local filter = database:smembers('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_)
...
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/Mortal)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
...
local filter = database:smembers('Mortal:'..bot_id.."filterphoto"..msg.chat_id_)
for k,v in pairs(filter) do
...
local filter = database:smembers('Mortal:'..bot_id.."filteranimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
------------------
if database:get('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
send(msg.chat_id_, msg.id_, 1, "*📬┇ تم الغاء الاذاعه للمشتركين *\n", 1, "md") 
database:del('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
----------
local pv = database:smembers('Mortal:'..bot_id.."userss")
local sendok = 0
...
----------
local gps = database:scard('Mortal:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = database:smembers('Mortal:'..bot_id..'userss') 
for k,v in pairs(list) do 
...
------------------
if is_sudo(msg) then 
if database:get("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
database:del("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_)
...
database:del("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('Mortal:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'Mortal:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('Mortal:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'Mortal:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('Mortal:'..bot_id..'pro:groups', gpss[i]) then
...
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
local keko2 = redis:get('Mortal:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('Mortal:'..bot_id..'keko'..keko2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('Mortal:'..bot_id..':sticker:'..keko2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('Mortal:'..bot_id..':voice:'..keko2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('Mortal:'..bot_id..':video:'..keko2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('Mortal:'..bot_id..':gif:'..keko2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('Mortal:'..bot_id..':file:'..keko2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('Mortal:'..bot_id..'kekore'..msg.chat_id_..'', keko2)
...
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 're' then
local keko2 = redis:get('Mortal:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('Mortal:'..bot_id..'keko'..keko2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('Mortal:'..bot_id..':sticker:'..keko2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('Mortal:'..bot_id..':voice:'..keko2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('Mortal:'..bot_id..':video:'..keko2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('Mortal:'..bot_id..':gif:'..keko2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('Mortal:'..bot_id..':file:'..keko2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('Mortal:'..bot_id..'kekoresudo', keko2)
send(msg.chat_id_, msg.id_, 1, "📊※ تم حفظ الرد  ✓", 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run Mortal                         ««              --
...
local svuser = 'Mortal:'..bot_id..'user:'..result.id_
if username then
...
if database:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:Mortal"..msg.chat_id_..bot_id) then
local pin_id = database:get('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local msgg = result 
database:incr('Mortal:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if database:get("lock_edit:Mortal"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/Mortal)" , 1, 'md') 
end
end
...
if database:get("lock_link.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end
end
...
if text:match("@") then
if database:get("lock_username.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
...
if text:match("#") then
if database:get("lock_tag.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
...
if text:match("/") then
if database:get("lock_sarha.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:Mortal"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
 end 
 end
...
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/MortalTEAM/Mortal/master/Mortal.lua") local g = io.open("Mortal.lua", 'w') g:write(a) g:close() dofile('Mortal.lua')
