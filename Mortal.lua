...
local files_Mortal = datahmade:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_Mortal) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_Mortal")
...
local hash = datahmade:sismember('Mortal:'..bot_id..'dev', msg.sender_user_id_) 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'creatorbasic:'..chat,user)
if hash then 
return true 
else 
return false 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'creator:'..chat, user) 
if hash then 
return true 
else 
return false 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_) 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)    
...
local hash = datahmade:sismember('Mortal:'..bot_id..'owners:'..chat, user) 
if hash then 
return true 
else 
return false 
...
function is_mod(msg)
local hash = datahmade:sismember('Mortal:'..bot_id..'mods:'..msg.chat_id_,msg.sender_user_id_)    
...
function is_vip(msg)
local hash = datahmade:sismember('Mortal:'..bot_id..'vipgp:'..msg.chat_id_,msg.sender_user_id_) 
...
local hash = datahmade:sismember('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_,msg.sender_user_id_) 
if hash then    
return true 
else 
return false 
...
var = true  
elseif datahmade:sismember('Mortal:'..bot_id..'dev', user_id) then
var = true  
elseif datahmade:sismember('Mortal:'..bot_id..'creatorbasic:'..chat_id, user_id) then
var = true
elseif is_creatorbasicc(chat_id, user_id) then
var = true
elseif datahmade:sismember('Mortal:'..bot_id..'creator:'..chat_id, user_id) then
var = true  
elseif datahmade:sismember('Mortal:'..bot_id..'owners:'..chat_id, user_id) then
var = true  
elseif datahmade:sismember('Mortal:'..bot_id..'mods:'..chat_id, user_id) then
var = true  
elseif datahmade:sismember('Mortal:'..bot_id..'vipgp:'..chat_id, user_id) then  
var = true  
else  
var = false  
end  
...
local var = false
local banned = datahmade:sismember('Mortal:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = datahmade:sismember('Mortal:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = datahmade:sismember('Mortal:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = datahmade:sismember('Mortal:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'Mortal:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
{
{text = ''..(channel_ts or "Mortal TEAM")..'', url=''..(channel_user_ts or 't.me/MortalTEAM')..''},
},
}
...
for Mortal_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = Mortal_one
break
end
end
end 
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'MortalTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_Mortal(Mortal1,Mortal2,Mortal3)
local id_Mortal = Mortal2.sender_user_id_
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'MortalTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_Mortal, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_Mortal)
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'MortalTEAM')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
function Mortal_run_file(data)
local files_Mortal = datahmade:smembers("files"..bot_id)
for i=1,#files_Mortal do
local Mortalee = dofile("files_Mortal/"..files_Mortal[i])
local kt = Mortalee.keko_Mortal(data)
if kt == 'end' then
return false
end
end
...
function Mortal_run_(msg,data)
local text = msg.content_.text_
------------------------------------------------------------------------

if text == ("مسح ردود المطور") and is_devahmad(msg) then
local list = redis:smembers('Mortal:'..bot_id..'sudo:kekore')
for k,v in pairs(list) do
redis:del('Mortal:'..bot_id.."sudo:add:reply1:gif"..v)   
redis:del('Mortal:'..bot_id.."sudo:add:reply1:vico"..v)   
redis:del('Mortal:'..bot_id.."sudo:add:reply1:stekr"..v)     
redis:del('Mortal:'..bot_id.."sudo:add:reply:rd"..v)   
redis:del('Mortal:'..bot_id.."sudo:addreply1:photo:gp"..v)
redis:del('Mortal:'..bot_id.."sudo:addreply1:video:gp"..v)
redis:del('Mortal:'..bot_id.."sudo:addreply1:document:gp"..v)
redis:del('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..v)
redis:del('Mortal:'..bot_id..'sudo:kekore')
end
...
for k,v in pairs(list) do
if redis:get('Mortal:'..bot_id.."sudo:add:reply1:gif"..v) then
db = 'متحركه 🎭'
elseif redis:get('Mortal:'..bot_id.."sudo:add:reply1:vico"..v) then
db = 'بصمه 📢'
elseif redis:get('Mortal:'..bot_id.."sudo:add:reply1:stekr"..v) then
db = 'ملصق 🃏'
elseif redis:get('Mortal:'..bot_id.."sudo:add:reply:rd"..v) then
db = 'رساله ✉'
elseif redis:get('Mortal:'..bot_id.."sudo:addreply1:photo:gp"..v) then
db = 'صوره 🎇'
elseif redis:get('Mortal:'..bot_id.."sudo:addreply1:video:gp"..v) then
db = 'فيديو 📹'
elseif redis:get('Mortal:'..bot_id.."sudo:addreply1:document:gp"..v) then
db = 'ملف 📁'
elseif redis:get('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..v) then
db = 'اغنيه 🎵'
end
...
local keko1 = redis:get('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('Mortal:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('Mortal:'..bot_id.."sudo:add:reply1:stekr"..test, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('Mortal:'..bot_id.."sudo:add:reply1:vico"..test, msg.content_.voice_.voice_.persistent_id_)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:voice:caption:gp"..test,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('Mortal:'..bot_id.."sudo:add:reply1:gif"..test, msg.content_.animation_.animation_.persistent_id_)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:animation:caption:gp"..test,(msg.content_.caption_ or ''))  
end  
if text then   
redis:set('Mortal:'..bot_id.."sudo:add:reply:rd"..test, text)  
end  
if msg.content_.audio_ then
redis:set('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..test, msg.content_.audio_.audio_.persistent_id_)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:audio:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('Mortal:'..bot_id.."sudo:addreply1:document:gp"..test, msg.content_.document_.document_.persistent_id_)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:document:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('Mortal:'..bot_id.."sudo:addreply1:video:gp"..test, msg.content_.video_.video_.persistent_id_)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:video:caption:gp"..test,(msg.content_.caption_ or ''))  
end
...
redis:set('Mortal:'..bot_id.."sudo:addreply1:photo:gp"..test, photo_in_group)  
redis:set('Mortal:'..bot_id.."sudo:addreply1:photo:caption:gp"..test,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
redis:set('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('Mortal:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('Mortal:'..bot_id.."sudo:add:reply1:gif"..text)   
redis:del('Mortal:'..bot_id.."sudo:add:reply1:vico"..text)   
redis:del('Mortal:'..bot_id.."sudo:add:reply1:stekr"..text)     
redis:del('Mortal:'..bot_id.."sudo:add:reply:rd"..text)   
redis:del('Mortal:'..bot_id.."sudo:addreply1:photo:gp"..text)
redis:del('Mortal:'..bot_id.."sudo:addreply1:video:gp"..text)
redis:del('Mortal:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:sadd('Mortal:'..bot_id..'sudo:kekore', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('Mortal:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:del('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('Mortal:'..bot_id..'sudo:kekore', text)
return false
end
end
if text == 'اضف رد للكل' and is_devahmad(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد للكل' and is_devahmad(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('Mortal:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not datahmade:get('Mortal:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local anemi = redis:get('Mortal:'..bot_id.."sudo:add:reply1:gif"..text)   
local veico = redis:get('Mortal:'..bot_id.."sudo:add:reply1:vico"..text)   
local stekr = redis:get('Mortal:'..bot_id.."sudo:add:reply1:stekr"..text)     
local text1 = redis:get('Mortal:'..bot_id.."sudo:add:reply:rd"..text)   
local photo = redis:get('Mortal:'..bot_id.."sudo:addreply1:photo:gp"..text)
local video = redis:get('Mortal:'..bot_id.."sudo:addreply1:video:gp"..text)
local document = redis:get('Mortal:'..bot_id.."sudo:addreply1:document:gp"..text)
local audio = redis:get('Mortal:'..bot_id.."sudo:addreply1:audio:gp"..text)
------------------------------------------------------------------------
local video_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:video:caption:gp"..text) or '' )
local photo_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:photo:caption:gp"..text) or '' )
local document_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:document:caption:gp"..text) or '' )
local audio_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:audio:caption:gp"..text) or '' )
local animation_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:animation:caption:gp"..text) or '' )
local voice_caption = (redis:get('Mortal:'..bot_id.."sudo:addreply1:voice:caption:gp"..text) or '' )
------------------------------------------------------------------------
if text1 then 
function Mortal_re(t1,t2)
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = datahmade:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي'
elseif is_sudo(msg) then
t = datahmade:get("Mortal:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
t = datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("Mortal:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg) then
t = datahmade:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
t = datahmade:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
getUser(msg.sender_user_id_, Mortal_re)
end
if stekr then 
...
local list = redis:smembers('Mortal:'..bot_id..'kekore'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('Mortal:'..bot_id.."add:reply1:gif"..v..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."add:reply1:vico"..v..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('Mortal:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_)
redis:del('Mortal:'..bot_id..'kekore'..msg.chat_id_..'')
end
...
for k,v in pairs(list) do
if redis:get('Mortal:'..bot_id.."add:reply1:gif"..v..msg.chat_id_) then
db = 'متحركه 🎭'
elseif redis:get('Mortal:'..bot_id.."add:reply1:vico"..v..msg.chat_id_) then
db = 'بصمه 📢'
elseif redis:get('Mortal:'..bot_id.."add:reply1:stekr"..v..msg.chat_id_) then
db = 'ملصق 🃏'
elseif redis:get('Mortal:'..bot_id.."add:reply:rd"..v..msg.chat_id_) then
db = 'رساله ✉'
elseif redis:get('Mortal:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_) then
db = 'صوره 🎇'
elseif redis:get('Mortal:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_) then
db = 'فيديو 📹'
elseif redis:get('Mortal:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_) then
db = 'ملف 📁'
elseif redis:get('Mortal:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_) then
db = 'اغنيه 🎵'
end
text = text.."<b>|"..k.."|</b>~⪼("..v..") » {"..db.."}\n"
if #text > 7000 then
...
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('Mortal:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('Mortal:'..bot_id.."add:reply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('Mortal:'..bot_id.."add:reply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
redis:set('Mortal:'..bot_id.."addreply1:voice:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('Mortal:'..bot_id.."add:reply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
redis:set('Mortal:'..bot_id.."addreply1:animation:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end  
if text then   
redis:set('Mortal:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
redis:set('Mortal:'..bot_id.."addreply1:audio:gp"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
redis:set('Mortal:'..bot_id.."addreply1:audio:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('Mortal:'..bot_id.."addreply1:document:gp"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
redis:set('Mortal:'..bot_id.."addreply1:document:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('Mortal:'..bot_id.."addreply1:video:gp"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
redis:set('Mortal:'..bot_id.."addreply1:video:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
redis:set('Mortal:'..bot_id.."addreply1:photo:gp"..test..msg.chat_id_, photo_in_group)  
redis:set('Mortal:'..bot_id.."addreply1:photo:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
if keko1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, ' 📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- #username > اسم المستخدم\n- #msgs > عدد رسائل المستخدم\n- #name > اسم المستخدم\n- #id > ايدي المستخدم\n- #stast > موقع المستخدم \n- #edit > عدد السحكات  ', 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('Mortal:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('Mortal:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('Mortal:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
redis:del('Mortal:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:sadd('Mortal:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('Mortal:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('Mortal:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:del('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('Mortal:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false
end
end
...
if res:match('"status":"left"') or res:match('USER_ID_INVALID') or res:match('kicked') then --@XOUXUfreer.Mortal ~= true then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
...
redis:set('Mortal:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not datahmade:get('Mortal:'..bot_id..'repowner:mute'..msg.chat_id_) then
local anemi = redis:get('Mortal:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
local veico = redis:get('Mortal:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
local stekr = redis:get('Mortal:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
local text1 = redis:get('Mortal:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
local photo = redis:get('Mortal:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
local video = redis:get('Mortal:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
local document = redis:get('Mortal:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
local audio = redis:get('Mortal:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
------------------------------------------------------------------------
local video_caption = (redis:get('Mortal:'..bot_id.."addreply1:video:caption:gp"..text..msg.chat_id_) or '' )
local photo_caption = (redis:get('Mortal:'..bot_id.."addreply1:photo:caption:gp"..text..msg.chat_id_) or '' )
local document_caption = (redis:get('Mortal:'..bot_id.."addreply1:document:caption:gp"..text..msg.chat_id_) or '' )
local audio_caption = (redis:get('Mortal:'..bot_id.."addreply1:audio:caption:gp"..text..msg.chat_id_) or '' )
local animation_caption = (redis:get('Mortal:'..bot_id.."addreply1:animation:caption:gp"..text..msg.chat_id_) or '' )
local voice_caption = (redis:get('Mortal:'..bot_id.."addreply1:voice:caption:gp"..text..msg.chat_id_) or '' )
------------------------------------------------------------------------
if text1 then 
function Mortal_re(t1,t2)
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
------------------------------------------------------------------------
if datahmade:get('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and is_sudo(msg) then 
...
datahmade:del('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
local gps = datahmade:scard('Mortal:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = datahmade:smembers('Mortal:'..bot_id..'userss') 
for k,v in pairs(list) do 
...
------------------------------------------------------------------------
if datahmade:get("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
datahmade:del("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = datahmade:scard('Mortal:'..bot_id..'pro:groups') or 0
if text then 
local gpss = datahmade:smembers( 'Mortal:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('Mortal:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = datahmade:smembers( 'Mortal:'..bot_id.."groups") or 0
for i=1, #gpss do
if not datahmade:sismember('Mortal:'..bot_id..'pro:groups', gpss[i]) then
...
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'texts'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'textst'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حفظ الاسم ', 1, 'html')
redis:set('Mortal:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'name_bot', text)
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^+(.*)$") then
local kekoo = redis:get('Mortal:'..bot_id..'sudoo'..text..'')
local keko2 = redis:get('Mortal:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('Mortal:'..bot_id..'nmkeko', text)
redis:set('Mortal:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
return false 
end
end
------------------------------------------------------------------------
...
redis:set('Mortal:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'nakeko', text)
local nmkeko = redis:get('Mortal:'..bot_id..'nmkeko')
...
datahmade:set("Mortal:gr:id:text:"..bot_id..msg.chat_id_,text)
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'h44'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'h33'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'h22'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'h11'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
if text and text:match("^(.*)$") then
local keko2 = redis:get('Mortal:'..bot_id..'hhh'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
------------------------------------------------------------------------
local keko2 = redis:get('Mortal:'..bot_id..'hres1'..msg.sender_user_id_..'')
if keko2 == 'msg' then
if text:match("^(.*)$") then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Mortal:'..bot_id..'hres1'..msg.sender_user_id_..'', 'no')
redis:set('Mortal:'..bot_id..'hres', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
...
if datahmade:get("lock_kansers:Mortal"..msg.chat_id_..bot_id) then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
local last_ = data.last_name_ or ''
local first_ = data.first_name_ or ''
taha = (first_..''..last_)
Num = (datahmade:get('Mortal:'..bot_id..'Num:kansers'..msg.chat_id_) or 25)
...
if datahmade:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_) then
text = datahmade:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
local taha = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)   
if taha then    
function get_info(arg,data)
if data.username_ ~= false then
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┇["..taha.."] \n" , 1, 'md') 
else
send(msg.chat_id_,0, 1, "⚠┇العضو : {["..data.first_name_.."](T.ME/MortalTEAM)}\n📛┇["..taha.."] \n" , 1, 'md') 
end
end
...
local filter = datahmade:smembers('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_)
...
send(msg.chat_id_,0, 1, "⚠┇عذرا يا » {["..data.first_name_.."](T.ME/MortalTEAM)}\n📛┇ الملصق الذي ارسلته تم منعه من المجموعه \n" , 1, 'md') 
end
end
getUser(msg.sender_user_id_,get_info)
delete_msg(msg.chat_id_,{[0] = msg.id_})       
...
local filter = datahmade:smembers('Mortal:'..bot_id.."filterphoto"..msg.chat_id_)
for k,v in pairs(filter) do
...
local filter = datahmade:smembers('Mortal:'..bot_id.."filteranimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if datahmade:get('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_) and datahmade:get("lock_pin:Mortal"..msg.chat_id_..bot_id) then
local pin_id = datahmade:get('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
if datahmade:get("lock_link.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
...
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
------------------------------------------------------------------------
...
if text and text:match("(.*)(@)(.*)")  then
if datahmade:get("lock_username.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if datahmade:get("lock_chat.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if datahmade:get("lock_sarha.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if datahmade:get("lock_tag.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_fwd.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_stecker.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_note.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_photo.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_audeo.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_voice.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_video.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_gif.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_contect.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_ar.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_files.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_en.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if datahmade:get("lock_mark.note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
------------------------------------------------------------------------
if text and text:match("(.*)") then
if datahmade:get("lock_word:Mortal"..msg.chat_id_..bot_id) then 
local Mortal_wr = (datahmade:get("Mortal:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(Mortal_wr) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if datahmade:get("lock_link:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
local text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if datahmade:get("lock_username:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
------------------------------------------------------------------------
if datahmade:get("lock_chat:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
------------------------------------------------------------------------
if text and text:match("(.*)(/)(.*)")  then
if datahmade:get("lock_sarha:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("(.*)(#)(.*)")  then
if datahmade:get("lock_tag:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if datahmade:get("lock_reple:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if  msg.content_.ID == "MessageChatJoinByLink" then
if datahmade:get("lock_join:Mortal"..msg.chat_id_..bot_id) then
...
if datahmade:get("lock_bot:Mortal"..msg.chat_id_..bot_id) and not bot_id then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if datahmade:get("lock_botAndBan:Mortal"..msg.chat_id_..bot_id) then
...
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if datahmade:get("lock_fwd:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if datahmade:get("lock_stecker:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if datahmade:get("lock_new:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageUnsupported" then
if datahmade:get("lock_note:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessagePhoto" then
if datahmade:get("lock_photo:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAudio" then
if datahmade:get("lock_audeo:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVoice" then
if datahmade:get("lock_voice:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageVideo" then
if datahmade:get("lock_video:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageAnimation" then
if datahmade:get("lock_gif:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageContact" then
if datahmade:get("lock_contact:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[\216-\219][\128-\191]") then
if datahmade:get("lock_ar:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if msg.content_.ID == "MessageDocument" then
if datahmade:get("lock_files:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
------------------------------------------------------------------------
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if datahmade:get("lock_en:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if datahmade:get("lock_mark:Mortal"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return false
end
end
...
if msg.content_.ID == "MessageChatAddMembers" then
datahmade:incr('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
if not is_vip(msg) then
if datahmade:get("lock_join:Mortal"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
return false
...
------------------------------------------------------------------------
if datahmade:get("lock_lllll:Mortal"..msg.chat_id_..bot_id) then
...
local hash = 'Mortal:'..bot_id..'flood:time:'..msg.chat_id_
if not datahmade:get(hash) then
floodTime = 1
else
...
datahmade:set("lock_media:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_note:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_word:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_mark:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_link:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set("lock_new:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:set('Mortal:'..bot_id..'get:photo'..msg.chat_id_,true)
...
datahmade:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
------------------------------------------------------------------------
if msg.content_.photo_ then
local photo = datahmade:get('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
if msg.content_.photo_.sizes_[0] then
...
datahmade:del('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false
end 
end
------------------------------------------------------------------------
...
if not datahmade:sismember( 'Mortal:'..bot_id.."groups",msg.chat_id_) then
datahmade:sadd( 'Mortal:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
datahmade:sadd('Mortal:'..bot_id.."userss",msg.chat_id_)
else
if not datahmade:sismember( 'Mortal:'..bot_id.."groups",msg.chat_id_) then
datahmade:sadd( 'Mortal:'..bot_id.."groups",msg.chat_id_)
end
...
datahmade:incr('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
------------------------------------------------------------------------
if datahmade:get('Mortal:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
datahmade:del('Mortal:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
if not datahmade:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
...
if datahmade:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:del( 'Mortal:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
local res = https.request('https://Mortal.gq/Mortal.php?id='..msg.sender_user_id_..'')
if res then
if res == 'false' then
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return 
...
info = '👤┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'Mortal')..')\n'
...
if database:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
...
if not database:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then

database:set( 'Mortal:'..bot_id.."charge:"..msg.chat_id_,true)
...
datahmade:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((datahmade:get("Mortal"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
datahmade:set( 'Mortal:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
datahmade:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
datahmade:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
...
local Mortale = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Mortale .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_Mortal/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if (text == 'الملفات' ) then
local files_Mortal = datahmade:smembers("files"..bot_id)
local files = io.popen('cd files_Mortal && ls'):read("*all")
local files_Mortal2 = ''
for i=1,#files_Mortal do
files_Mortal2 = files_Mortal2..'{'..files_Mortal[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..files..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_Mortal2..'', 1, 'html')
end
...
local f = load("files_Mortal/"..name_t[2]..".lua")
...
if (f:match('^(.*)(22Mortal)(.*)(bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster)(.*)(Mortal)(.*)(elua)(.*)$')) then 
ts23kk =  "\n⚠️┇عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(MortalTEAM/Mortal)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على الفرخ رضا اللوكي + كيكو دروح اتلوك بعد 👻😂 \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -fr Mortal.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف Mortal.lua لتبعبص ابني "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(Mortal.lua)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(22Mortal%3a%22%20%2e%2e%20bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster%2fMortal%2elua)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
datahmade:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Mortal/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
io.popen("rm -fr files_Mortal/del.lua")
io.popen("rm -fr files_Mortal/hack.lua")
io.popen("rm -fr files_Mortal/d.lua")
if f ~= "true" then
ts23 = ""
if f:match("^(.*)(keko_Mortal)(.*)$") then
...
datahmade:del('Mortal:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
if text == ("مسح المكتومين عام") then
text = '☑┇ تم مسح المكتومين عام'
datahmade:del('Mortal:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
------------------------------------------------------------------------
if text == ("قائمه العام") then
local hash =   'Mortal:'..bot_id..'gbanned:'
local list = datahmade:smembers(hash)
text = "⛔┇قائمة الحظر العام  ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('Mortal:'..bot_id..'user:'..v)
...
local hash =   'Mortal:'..bot_id..'gmuted:'
local list = datahmade:smembers(hash)
...
local hash =  'Mortal:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'MortalTEAM')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'Mortal:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'MortalTEAM')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
if redis:sismember('Mortal:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'🔖┇بالفعل تم رفعة مطور في البوت')
else
redis:set('Mortal:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('Mortal:'..bot_id..'dev', result.sender_user_id_)
...
redis:set('Mortal:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('Mortal:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MortalTEAM')..')\n\n🔖┇تم رفعة مطور في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
redis:set('Mortal:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('Mortal:'..bot_id..'dev', apmd[2])
...
if not redis:sismember('Mortal:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'بالفعل تم تنزيلة من مطورين البوت')
else
redis:del('Mortal:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('Mortal:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('Mortal:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('Mortal:'..bot_id..'dev', result.id_)
...
redis:del('Mortal:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('Mortal:'..bot_id..'dev', apmd[2])
...
os.execute('rm -rf Mortal.lua')
os.execute('cd libs;wget https://raw.githubusercontent.com/Mortalahmad/Mortal/master/libs/utils.lua')
os.execute('wget https://raw.githubusercontent.com/Mortalahmad/Mortal/master/Mortal.lua')
dofile('Mortal.lua')  
return false
end
...
local gps = datahmade:scard('Mortal:'..bot_id.."groups")  
...
redis:del('Mortal:'..bot_id..'name_bot')
...
local gps = datahmade:scard('Mortal:'..bot_id.."userss")  
...
local pv = datahmade:smembers('Mortal:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
datahmade:srem('Mortal:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = datahmade:smembers('Mortal:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
...
datahmade:srem('Mortal:'..bot_id..'pro:groups',group[i]) 
datahmade:srem( 'Mortal:'..bot_id.."groups",group[i]) 
...
local gps = datahmade:smembers('Mortal:'..bot_id.."groups") or 0
local gps2 = datahmade:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
datahmade:sadd("thsake:gog"..bot_id, gps[i])
datahmade:set('Mortal:'..bot_id.."enable:"..gps[i],true)
datahmade:set( 'Mortal:'..bot_id.."charge:"..gps[i],true)
end
...
datahmade:del('Mortal:'..bot_id.."enable:"..gps[i])
datahmade:del( 'Mortal:'..bot_id.."charge:"..gps[i])
end
...
datahmade:del("thsake:gog"..bot_id)
datahmade:del('Mortal:'..bot_id.."groups")
end
------------------------------------------------------------------------
...
for i=1, #gpss do
local link = datahmade:get('Mortal:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('Mortal.txt', 'w')
f:write(text233)
f:close()
local Mortale = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Mortale .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'Mortal.txt' .. '"'
io.popen(curl)
end
------------------------------------------------------------------------
...
redis:set('Mortal:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false 
end
if text == ("مسح المطورين") then
local list = redis:smembers('Mortal:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('Mortal:'..bot_id..'dev')
redis:del('Mortal:'..bot_id..'sudoo'..v)
end
...
if datahmade:sismember('Mortal:'..bot_id..'pro:groups',msg.chat_id_) then
...
datahmade:sadd('Mortal:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') then
if not datahmade:sismember('Mortal:'..bot_id..'pro:groups',msg.chat_id_) then
...
datahmade:srem('Mortal:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تفعيل الاذاعه') then
if not datahmade:get('Mortal:'..bot_id..'bc:groups') then
...
datahmade:del('Mortal:'..bot_id..'bc:groups')
end
end
if (text and text == 'تعطيل الاذاعه') then
if datahmade:get('Mortal:'..bot_id..'bc:groups') then
...
datahmade:set('Mortal:'..bot_id..'bc:groups',true)
end
end
if text and text == "تعين مجموعه المطور" then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين ~⪼ `'..msg.chat_id_..'`', 1, 'md')
datahmade:set("Mortal"..bot_id..":sudo:gr",msg.chat_id_)
end
if text and text == "مسح مجموعه المطور" then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم مسح مجموعه المطور', 1, 'md')
datahmade:del("Mortal"..bot_id..":sudo:gr")
end
if (text and text == 'استعاده الاوامر') then
redis:del('Mortal:'..bot_id..'help', text)
redis:del('Mortal:'..bot_id..'h1', text)
redis:del('Mortal:'..bot_id..'h2', text)
redis:del('Mortal:'..bot_id..'h3', text)
redis:del('Mortal:'..bot_id..'h4', text)
redis:del('Mortal:'..bot_id..'h5', text)
redis:del('Mortal:'..bot_id..'h6', text)
redis:del('Mortal:'..bot_id..'hres')
send(msg.chat_id_, msg.id_, 1, '☑┇تم استعاده جميع كلايش الاوامر', 1, 'html')
end
if (text and text == 'تغير امر الاوامر') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Mortal:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end


if (text and text == 'تغير امر م1') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Mortal:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
------------------------------------------------------------------------
...
redis:set('Mortal:'..bot_id..'hres1'..msg.sender_user_id_..'', 'msg')
return false 
end

if (text and text == 'تغير امر م2') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Mortal:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م3') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Mortal:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م4') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Mortal:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'حذف كليشه المطور') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('Mortal:'..bot_id..'text_sudo', text)
...
if (not datahmade:get('Mortal:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
datahmade:set("Mortal:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "Mortal"
else
...
if (not datahmade:get('Mortal:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
datahmade:setex('Mortal:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
if not datahmade:get( 'Mortal:'..bot_id.."charge:"..msg.chat_id_) then
datahmade:set( 'Mortal:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."✔️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,thsake_info)
...
datahmade:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((datahmade:get("Mortal"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n📌┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
------------------------------------------------------------------------
if text == 'مغادره' then
local lave = datahmade:get('Mortal:'..bot_id..'leave:groups')
print(lave)
if not datahmade:get('Mortal:'..bot_id..'leave:groups') then
...
datahmade:del('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
...
function promote_by_reply(extra, result, success)
datahmade:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
datahmade:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
datahmade:sadd('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
return false end
function promote_by_reply(extra, result, success)
datahmade:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
datahmade:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"تم تنزيلة منشئ اساسي من البوت")
return false
end
------------------------------------------------------------------------
if (text == ("عدد الكروبات") or text ==("الاحصائيات")) then
local gps = datahmade:scard('Mortal:'..bot_id.."groups") or 0
local user = datahmade:scard('Mortal:'..bot_id.."userss") or 0
local gps2 = datahmade:scard("thsake:gog"..bot_id) or 0
local gps9 = datahmade:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = datahmade:scard('Mortal:'..bot_id..'pro:groups') or 0
...
local time = a[2] * day
datahmade:setex( 'Mortal:'..bot_id.."charge:"..msg.chat_id_,time,true)
datahmade:set( 'Mortal:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = datahmade:ttl( 'Mortal:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
datahmade:setex( 'Mortal:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
datahmade:set( 'Mortal:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
------------------------------------------------------------------------
...
datahmade:setex( 'Mortal:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
datahmade:set( 'Mortal:'..bot_id.."charge:"..txt[2],true)
...
local list = datahmade:smembers('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
...
local hash =  'Mortal:'..bot_id..'creator:'..msg.chat_id_
...
datahmade:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'MortalTEAM')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
datahmade:sadd('Mortal:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
datahmade:del('Mortal:'..bot_id..'creator:'..msg.chat_id_)
...
local tsX_o = datahmade:get("lock_edit:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
datahmade:set("lock_edit:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل بالطرد") then
local tsX_o = datahmade:get("lock_edit:Mortal:kick"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل بالطرد")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل بالطرد")
datahmade:set("lock_edit:Mortal:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_edit:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_edit:Mortal:kick"..msg.chat_id_..bot_id,"ok")
end
end
if text == 'مسح المدراء' then
datahmade:del('Mortal:'..bot_id..'owners:'..msg.chat_id_)
...
local hash =  'Mortal:'..bot_id..'owners:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
...
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
datahmade:set("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_,"MortalE")
send(msg.chat_id_, msg.id_, 1, "🔌┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ") then 
datahmade:del("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
datahmade:del('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
datahmade:set('Mortal:'..bot_id.."Mortal:lock:set"..msg.chat_id_,"Mortal")
...
datahmade:del('Mortal:'..bot_id.."Mortal:lock:set"..msg.chat_id_)
...
datahmade:set("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_,"Mortal")
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تفعيل الطرد" or text == "تفعيل الحظر")) then
datahmade:del("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
datahmade:set("Mortal:name_own"..bot_id..msg.chat_id_,ac[1]) 
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
------------------------------------
if text and text:match("^تغير رد الادمن (.*)$") then
ac = {string.match(text, "^تغير رد الادمن (.*)$")}
datahmade:set("Mortal:name_adm"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if text and text:match("^تغير رد العضو المميز (.*)$") then
ac = {string.match(text, "^تغير رد العضو المميز (.*)$")}
datahmade:set("Mortal:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text and text:match("^تغير رد العضو فقط (.*)$") then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
datahmade:set("Mortal:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه") then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
datahmade:set("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Mortal"
end
if text and datahmade:get("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
datahmade:del("Mortal:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
datahmade:set("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
datahmade:set("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Mortal"
end
if (text and datahmade:get("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
datahmade:del("Mortal:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = datahmade:get("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
datahmade:set("Mortal:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
datahmade:del("Mortal:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '??┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
datahmade:set('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
datahmade:sadd('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
...
datahmade:sadd('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
datahmade:srem('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
datahmade:srem('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
datahmade:del('Mortal:'..bot_id..'SET:BAN'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح قائمه صلاحيات الحظر', 1, 'md')
end
if text == ("قائمه صلاحيه الحظر") then
local hash =  'Mortal:'..bot_id..'SET:BAN'..msg.chat_id_
local list = datahmade:smembers(hash)
...
local Mortal_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
datahmade:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,Mortal_edis[1])
...
datahmade:set("Mortal:edit:text:su:new:"..bot_id..msg.chat_id_..datahmade:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
datahmade:set("Mortal:edit:text:su:new2:"..bot_id..msg.chat_id_..text,datahmade:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = datahmade:get("Mortal:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
datahmade:del("Mortal:edit:text:su:new2:"..bot_id..msg.chat_id_..klma)
datahmade:del("Mortal:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
datahmade:set('Mortal:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and datahmade:get('Mortal:'..bot_id.."Mortal:lock:set"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false
end
function promote_by_reply(extra, result, success)
local hash =  'Mortal:'..bot_id..'mods:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
...
datahmade:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
datahmade:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
datahmade:del('Mortal:'..bot_id..'mods:'..msg.chat_id_)
...
datahmade:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'Mortal')
return "Mortal"
end

if text and text == "مسح الايدي" then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
datahmade:del("Mortal:gr:id:text:"..bot_id..msg.chat_id_)
end
...
datahmade:set('Mortal:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
datahmade:set('Mortal:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
...
i = i + 1
local user_info = datahmade:hgetall('Mortal:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
if not datahmade:get('Mortal:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني بالفعل تم تفعيلة ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني تم تفعيلة ', 1, 'md')
datahmade:del('Mortal:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') then
if datahmade:get('Mortal:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني بالفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني ', 1, 'md')
datahmade:set('Mortal:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end
if text and text:match("^رفع (.*)$")  and msg.reply_to_message_id_ then
a = {string.match(text, "^رفع (.*)$")}
ts_text = datahmade:get("Mortal:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
datahmade:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
datahmade:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
datahmade:set("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function keko33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'MortalTEAM')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = datahmade:get("Mortal:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
...
datahmade:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
datahmade:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
datahmade:set("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MortalTEAM')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
datahmade:sadd('Mortal:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
datahmade:sadd('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
datahmade:sadd('Mortal:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
datahmade:set("Mortal:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
datahmade:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
datahmade:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
datahmade:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
datahmade:del("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if ts_text == "ادمن" then 
datahmade:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
datahmade:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
datahmade:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MortalTEAM')..')\n🔰┇تم تنزيله من '..apmd[1]
datahmade:del("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
datahmade:sadd('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
datahmade:sadd('Mortal:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
datahmade:sadd('Mortal:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
datahmade:srem('Mortal:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
datahmade:srem('Mortal:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
datahmade:srem('Mortal:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
...
datahmade:set('Mortal:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
...
datahmade:del('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
local tsX_o = datahmade:get("lock_tag:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
datahmade:set("lock_tag:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = datahmade:get("lock_reple:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
datahmade:set("lock_reple:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = datahmade:get("lock_sarha:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
datahmade:set("lock_sarha:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = datahmade:get("lock_word:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_lllll:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
datahmade:set("lock_lllll:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = datahmade:get("lock_gif:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_files:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
datahmade:set("lock_files:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = datahmade:get("lock_mark:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_photo:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_stecker:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_video:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_inline:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
datahmade:set("lock_inline:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = datahmade:get("lock_chat:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
datahmade:set("lock_chat:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = datahmade:get("lock_fwd:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
datahmade:set("lock_fwd:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = datahmade:get("lock_pin:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
datahmade:set("lock_pin:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = datahmade:get("lock_audeo:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_voice:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_contact:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
datahmade:set("lock_contact:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = datahmade:get("lock_ar:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
datahmade:set("lock_ar:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = datahmade:get("lock_en:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
datahmade:set("lock_en:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = datahmade:get("lock_media:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_link:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_username:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
datahmade:set("lock_username:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = datahmade:get("lock_new:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = datahmade:get("lock_botAndBan:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
datahmade:set("lock_botAndBan:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = datahmade:get("lock_bot:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
datahmade:set("lock_bot:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = datahmade:get("lock_join:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
datahmade:set("lock_join:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = datahmade:get("lock_note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
...
datahmade:del("lock_tag:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_sarha:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_word:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_lllll:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_files:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_mark:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_contact:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_fwd:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_link:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_username:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_botAndBan:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_new:Mortal"..msg.chat_id_..bot_id,"ok")
...
datahmade:del("lock_reple:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_word:Mortal"..msg.chat_id_..bot_id)
end
end
...
datahmade:del("lock_inline:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_chat:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_pin:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_ar:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_en:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_media:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_audeo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_video:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_photo:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_stecker:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_voice:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_gif:Mortal"..msg.chat_id_..bot_id,"ok")
datahmade:del("lock_note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_join:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_bot:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
if text and text:match('^وضع عدد الكانسر (%d+)$') then
local Num = text:match('^وضع عدد الكانسر (%d+)$')
datahmade:set('Mortal:'..bot_id..'Num:kansers'..msg.chat_id_,Num)
send(msg.chat_id_, msg.id_, 1, '\n🔖┇تم وضع عدد حروف الاسم {'..Num..'} حرف',1, 'md')
end
if (text == "قفل الكانسر") then
local tsX_o = datahmade:get("lock_kansers:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكانسر ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكانسر ")
datahmade:set("lock_kansers:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
datahmade:del("lock_kansers:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
...
return false end
function promote_by_reply(extra, result, success)
local hash =  'Mortal:'..bot_id..'vipgp:'..msg.chat_id_
if datahmade:sismember(hash, result.sender_user_id_) then
...
if not is_creator(msg) and not is_setban(msg) and datahmade:get("Mortal:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'Mortal:'..bot_id..'banned:'..msg.chat_id_
...
datahmade:sadd('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'MortalTEAM')..')\n☑┇تم حظره من المجموعه'
...
datahmade:sadd('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'Mortal:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'Mortal:'..bot_id..'banned:'..msg.chat_id_
...
datahmade:del('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
datahmade:del('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'MortalTEAM')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
datahmade:del('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
datahmade:del('Mortal:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
datahmade:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
datahmade:srem('Mortal:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
datahmade:srem('Mortal:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
function mute_by_reply(extra, result, success)
local hash =  'Mortal:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) == true then
...
datahmade:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'MortalTEAM')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
datahmade:sadd('Mortal:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
datahmade:srem('Mortal:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
datahmade:srem('Mortal:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'MortalTEAM')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
local Mortal_098 = 0
if datahmade:get("Mortal:get:mod:"..bot_id..msg.chat_id_) then 
t = datahmade:get("Mortal:get:mod:"..bot_id..msg.chat_id_)
Mortal_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..Mortal_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
datahmade:set("Mortal:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
datahmade:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,text)
...
local wel = datahmade:get('Mortal:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
Mortal_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..Mortal_0[2]..'`}', 1, 'md')
datahmade:set("Mortal:not:word:"..bot_id..msg.chat_id_,Mortal_0[2])
end

if text == ("الاعدادات") then
if datahmade:get("lock_media:Mortal"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not datahmade:get('Mortal:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = datahmade:get('Mortal:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not datahmade:get( 'Mortal:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = datahmade:get( 'Mortal:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if datahmade:get("lock_audeo:Mortal"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if datahmade:get("lock_bot:Mortal"..msg.chat_id_..bot_id) then
mute_bots = '✔┇'
else
mute_bots = '✖┇'
end
...
------------
if datahmade:get("lock_inline:Mortal"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if datahmade:get("lock_edit:Mortal"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if datahmade:get("lock_pin:Mortal"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if datahmade:get('Mortal:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
...
------------
local ex = datahmade:ttl( 'Mortal:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local list = datahmade:smembers('Mortal:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)  
datahmade:srem('Mortal:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
...
for k,v in pairs(list) do  
local taha = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
datahmade:set('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
datahmade:set('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
datahmade:set('Mortal:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
datahmade:sadd('Mortal:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
datahmade:set('Mortal:'..bot_id.."filtr1:add:reply:rd"..test..msg.chat_id_, text)  
datahmade:hset('Mortal:'..bot_id..'filters:'..msg.chat_id_, text, 'filtered')
end  
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
...
datahmade:set('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = datahmade:get('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
datahmade:del('Mortal:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)  
datahmade:srem('Mortal:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
...
datahmade:set('Mortal:'..bot_id..'rules'..msg.chat_id_, txt[2])
...
datahmade:set('Mortal:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
local hash = 'Mortal:'..bot_id..'res:'..msg.chat_id_
local list = datahmade:smembers(hash)
text = "👥┇قائمة المقيدين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = datahmade:hgetall('Mortal'..bot_id.."user:"..v)
...
local hash = 'Mortal:'..bot_id..'res:'..msg.chat_id_
local list = datahmade:smembers(hash) 
for k,v in pairs(list) do datahmade:del('Mortal:'..bot_id..'res:'..msg.chat_id_) 
...
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(res[2] or 'Mortalteam')..')\n☑️┇تم تقييده'
end
else
texts = '✖️┇خطاء'
end
...
datahmade:sadd('Mortal:'..bot_id..'res:'..msg.chat_id_, apba[2])
...
local tsX_o = datahmade:get("lock_tag.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التاك بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التاك بالتقييد")
datahmade:set("lock_tag.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الشارحه بالتقيد") then
local tsX_o = datahmade:get("lock_sarha.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الشارحه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الشارحه بالتقييد")
datahmade:set("lock_sarha.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المتحركه بالتقيد") then
local tsX_o = datahmade:get("lock_gif.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل المتحركه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المتحركه بالتقييد")
datahmade:set("lock_gif.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملفات بالتقيد") then
local tsX_o = datahmade:get("lock_files.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملفات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملفات بالتقييد")
datahmade:set("lock_files.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الماركدون بالتقيد") then
local tsX_o = datahmade:get("lock_mark.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الماكدون بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الماركدون بالتقييد")
datahmade:set("lock_mark.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصور بالتقيد") then
local tsX_o = datahmade:get("lock_photo.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصور بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصور بالتقييد")
datahmade:set("lock_photo.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملصقات بالتقيد") then
local tsX_o = datahmade:get("lock_stecker.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملصقات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملصقات بالتقييد")
datahmade:set("lock_stecker.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الفيديو بالتقيد") then
local tsX_o = datahmade:get("lock_video.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الفيديو بالتقييد")
datahmade:set("lock_video.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانلاين بالتقيد") then
local tsX_o = datahmade:get("lock_inline.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانلاين بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانلاين بالتقييد")
datahmade:set("lock_inline.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الدردشه بالتقيد") then
local tsX_o = datahmade:get("lock_chat.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الدردشه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الدردشه بالتقييد")
datahmade:set("lock_chat.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل التوجيه بالتقيد") then
local tsX_o = datahmade:get("lock_fwd.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التوجيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التوجيه بالتقييد")
datahmade:set("lock_fwd.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الاغاني بالتقيد") then
local tsX_o = datahmade:get("lock_audeo.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الاغاني بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الاغاني بالتقييد")
datahmade:set("lock_audeo.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصوت بالتقيد") then
local tsX_o = datahmade:get("lock_voice.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصوت بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصوت بالتقييد")
datahmade:set("lock_voice.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الجهات بالتقيد") then
local tsX_o = datahmade:get("lock_contact.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الجهات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الجهات بالتقييد")
datahmade:set("lock_contact.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل العربيه بالتقيد") then
local tsX_o = datahmade:get("lock_ar.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل العربيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل العربيه بالتقييد")
datahmade:set("lock_ar.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانكليزيه بالتقيد") then
local tsX_o = datahmade:get("lock_en.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانكليزيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانكليزيه بالتقييد")
datahmade:set("lock_en.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الروابط بالتقيد") then
local tsX_o = datahmade:get("lock_link.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الروابط بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الروابط بالتقييد")
datahmade:set("lock_link.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المعرف بالتقيد") then
local tsX_o = datahmade:get("lock_username.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
 tsX000("lock",msg,"☑️┇بالفعل تم قفل المعرف بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المعرف بالتقييد")
datahmade:set("lock_username.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل بصمه الفيديو بالتقيد") then
local tsX_o = datahmade:get("lock_note.note:Mortal"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل بصمه الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل بصمه الفيديو بالتقييد")
datahmade:set("lock_note.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_tag.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_sarha.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_gif.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_files.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_mark.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_photo.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_stecker.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_video.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_inline.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_chat.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_fwd.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_audeo.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_voice.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_contact.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_ar.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_en.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_username.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_note.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
datahmade:del("lock_link.note:Mortal"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
if datahmade:get("lock_audeo.note:Mortal"..msg.chat_id_..bot_id) then
 ...
 ------------
if datahmade:get("lock_inline.note:Mortal"..msg.chat_id_..bot_id) then
 mute_in = '✔┇'
else
 mute_in = '✖┇'
 end
...
lock_tag = '✖┇'
 ...
 ------------
if datahmade:get("lock_contact.note:Mortal"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
 end
...
datahmade:del('Mortal:'..bot_id..'banned:'..msg.chat_id_)
...
if datahmade:sismember('Mortal:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif datahmade:sismember('Mortal:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif datahmade:sismember('Mortal:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif datahmade:sismember('Mortal:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
if redis:sismember('Mortal:'..bot_id..'dev',result.id_) then
dev = 'المطور ،' else dev = '' end
if datahmade:sismember('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if datahmade:sismember('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.id_) then
cr = 'منشئ ،' else cr = '' end
if datahmade:sismember('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_) then
own = 'مدير ،' else own = '' end
if datahmade:sismember('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_) then
mod = 'ادمن ،' else mod = '' end
if datahmade:sismember('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_) then
vip = 'مميز ،' else vip = ''
end
...
datahmade:srem('Mortal:'..bot_id..'dev', result.id_)
datahmade:del('Mortal:'..bot_id..'sudoo'..result.id_..'', 'no')
datahmade:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_,result.id_)
datahmade:srem('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.id_)
datahmade:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.id_)
datahmade:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.id_)
datahmade:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
if redis:sismember('Mortal:'..bot_id..'dev',result.sender_user_id_) then
dev = 'المطور ،' else dev = '' end
if datahmade:sismember('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if datahmade:sismember('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_) then
cr = 'منشئ ،' else cr = '' end
if datahmade:sismember('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_) then
own = 'مدير ،' else own = '' end
if datahmade:sismember('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_) then
mod = 'ادمن ،' else mod = '' end
if datahmade:sismember('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_) then
vip = 'مميز ،' else vip = ''
end
...
if RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo1' then
datahmade:srem('Mortal:'..bot_id..'dev', result.sender_user_id_)
datahmade:del('Mortal:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
datahmade:srem('Mortal:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
datahmade:srem('Mortal:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('Mortal:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('Mortal:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
datahmade:srem('Mortal:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo' then
...
------------------------------------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not datahmade:get("Mortal:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = datahmade:get("Mortal:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
datahmade:set("Mortal:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
t = datahmade:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = datahmade:get("Mortal:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = datahmade:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = datahmade:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
...
if not datahmade:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = datahmade:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
Mortal_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
Mortal_oop = datahmade:get("Mortal:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
Mortal_oop = datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
Mortal_oop = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
Mortal_oop = datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
Mortal_oop = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
Mortal_oop = datahmade:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
Mortal_oop = datahmade:get("Mortal:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
Mortal_oop = datahmade:get("Mortal:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
Mortal_oop = datahmade:get("Mortal:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
if result.id_ then
if not datahmade:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = datahmade:get('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not datahmade:get("Mortal:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (datahmade:get('Mortal:'..bot_id.."group:link"..msg.chat_id_) and datahmade:get('Mortal:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '*📮┇𝗚𝗿𝘂𝗼𝗽 𝗟𝗶𝗹𝗻𝗸* \n['..datahmade:get('Mortal:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
...
datahmade:set('Mortal:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
local user_msgs = datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if not datahmade:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
add = (tonumber(datahmade:get('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
datahmade:del('Mortal:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
datahmade:del('Mortal:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local Mortal = '🗑 ┇ تم حذف سحكاتك '
send(msg.chat_id_, msg.id_, 1, Mortal, 1, 'md')
end
------------------------------------------------------------------------
if text == ("اطردني") then
local res = https.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot1170522933:AAEGNWVfIfhWrJlaxQaALddEIAOr7z39lc0/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot666998640:AAEOwMMQU8pzy60wLF23EiXjbfVsI6jrQAk/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://api.telegram.org/bot827950325:AAGjaAxIwupH9vWLvB3JIRZRGFHhSVZsM9Y/getChatMember?chat_id=@XOUXU&user_id='..msg.sender_user_id_)--ttps.request('https://teamstorm.tk/joinch/?id='..msg.sender_user_id_..'')
...
return false end
if not datahmade:get('Mortal:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('Mortal:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('Mortal:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
local yess = redis:get('Mortal:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('Mortal:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Mortal:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ', 1, 'md')
end
end
end
if text and text:match("^لا$") then
local noo = redis:get('Mortal:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('Mortal:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Mortal:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
------------------------------------------------------------------------
if (text and text == (redis:get('Mortal:'..bot_id..'name_bot') or 'سنايبر'))then
name_bot = (redis:get('Mortal:'..bot_id..'name_bot') or 'سنايبر')
local namebot = {
...
local text_sudo = redis:get('Mortal:'..bot_id..'text_sudo')
local nkeko = redis:get('Mortal:'..bot_id..'nmkeko')
local nakeko = redis:get('Mortal:'..bot_id..'nakeko')
if text_sudo then
...
local rules = datahmade:get('Mortal:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @XOUXU ⚜️\n', 1, 'html')   
return false end
if not datahmade:sismember('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
datahmade:sadd('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_Mortal = result.total_count_ 
local tahna = (datahmade:get('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((datahmade:get('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل👻"
if user_msgs then 
...
t = 'آلمطور آلاساسي 👨🏻‍💻'
elseif is_sudo(msg) then
t = datahmade:get("Mortal:name_sudo"..bot_id..msg.chat_id_)  or 'مطور مميز 👨🏽‍🔧'
elseif is_creatorbasic(msg) then
t =  datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t =  datahmade:get("Mortal:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and datahmade:get("Mortal:all_if:"..datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = datahmade:get("Mortal:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = datahmade:get("Mortal:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵'   
...
if not datahmade:get('Mortal:'..bot_id..'id:mute'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'id:photo'..msg.chat_id_) then
if not datahmade:get("Mortal:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'يعني الوك الزينه 👨🏻‍🚒❤️  ',
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه ",
...
local Mortal_new_text = datahmade:get("Mortal:gr:id:text:"..bot_id..msg.chat_id_)
local Mortal_new_text = Mortal_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#edit',(edit or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#stast',(t or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#photos',(all_photo_Mortal or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#game',(nko or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,Mortal_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'آلمطور آلاساسي 👨🏻‍💻'
...
local Mortal_new_text = Mortal_new_text:gsub('#photos',(all_photo_Mortal or 'لا يوجد'))
local Mortal_new_text = Mortal_new_text:gsub('#game',(nko or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, Mortal_new_text, 1, 'html')
end   
end
else
end
...
------------------------------------------------------------------------
if text then
if datahmade:sismember('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
datahmade:del('Mortal:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
send(msg.chat_id_, msg.id_, 1, '📜┇معرف ~⪼ ('..(result.username_..')\n🔘┇الاسم ~⪼ ('..result.first_name_ or "Mortal")..')', 1, 'md')
end
getUser(memb[2],whois)
end
------------------------------------------------------------------------
if text == ("مشاهده منشور") then
datahmade:set('Mortal:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
------------------------------------------------------------------------
if text == 'سمايلات' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🥖','🥐','🍞','🥨','🍟','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍠','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','??','??','🏸','🥅','🎰','??','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','??','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..'klmos'..msg.chat_id_,name)
taha = '🚀┇اسرع واحد يدز » {`'..name..'`}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('Mortal:'..bot_id..'l:ids'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*🎉¦ مبروك لقد ربحت نقطه ,\n🎮| للعب مره اخرى ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('Mortal:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'الاسرع' or text =='اسرع' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..'klmo'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('Mortal:'..bot_id..'l:id'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*🎉¦ مبروك لقد ربحت نقطه ,\n🎮| للعب مره اخرى ارسل الاسرع ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1) 
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('Mortal:'..bot_id..'l:id'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'حزوره' or text =='حزورة' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..'klmoa'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('Mortal:'..bot_id..'l:id1'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*🎉¦ مبروك لقد ربحت نقطه ,\n🎮| للعب مره اخرى ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('Mortal:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'معاني' or text =='المعاني' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..'means'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('Mortal:'..bot_id..'l:id2'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*🎉¦ مبروك لقد ربحت نقطه ,\n🎮| للعب مره اخرى ارسل معاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('Mortal:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'ترتيب' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:id2'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == 'العكس' or text =='عكس' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
datahmade:del('Mortal:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..'aks'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not datahmade:get('Mortal:'..bot_id..'l:id3'..msg.chat_id_) then
if not datahmade:get('Mortal:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*🎉¦ مبروك لقد ربحت نقطه ,\n🎮| للعب مره اخرى ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
datahmade:set('Mortal:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if datahmade:get('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = datahmade:get('Mortal:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
datahmade:del('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖┇ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸┇ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
datahmade:incrby('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(datahmade:get('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
datahmade:del('Mortal:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if datahmade:get('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = datahmade:get('Mortal:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
datahmade:del('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮┇ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊┇ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
datahmade:del('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if text == 'المختلف' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
datahmade:del('Mortal:'..bot_id..'l:idss'..msg.chat_id_)
datahmade:set('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == 'امثله' and datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
datahmade:set('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
datahmade:del('Mortal:'..bot_id..'l:idst'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_) or '57999')..'' then -- // المختلف
if not datahmade:get('Mortal:'..bot_id..'l:idss'..msg.chat_id_) then 
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
datahmade:del('Mortal:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎉¦ آحسنت لقد ربحت نقطه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
datahmade:set('Mortal:'..bot_id..'l:idss'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == ''..(datahmade:get('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_) or '6499')..'' then -- // امثله
if not datahmade:get('Mortal:'..bot_id..'l:idst'..msg.chat_id_) then 
datahmade:incrby('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
datahmade:incrby('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
datahmade:del('Mortal:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎉¦ آحسنت لقد ربحت نقطه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
datahmade:set('Mortal:'..bot_id..'l:idst'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == 'محيبس' or text == 'بات' then
if datahmade:get('Mortal:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
datahmade:set('Mortal:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
datahmade:setex('Mortal:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
...
datahmade:set('Mortal:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n🎮┇ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠┇ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖┇ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n💥*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
datahmade:setex('Mortal:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
if text =='مجوهراتي' then 
if tonumber((datahmade:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💬┇ليس لديك مجوهرات ، \n💎┇للحصول ؏ مجوهرات ، \n📮┇ارسل الالعاب وابدأ اللعب !*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎┇ عدد مجوهراتك الحاليه  ('..(datahmade:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎┇ مجموع مجوهراتك  ('..(datahmade:get('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
taha = (datahmade:get('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(datahmade:get('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
datahmade:incrby('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
datahmade:del('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((datahmade:get('Mortal:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
...
datahmade:del('Mortal:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
datahmade:del('Mortal:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
------------------------------------------------------------------------
if text == 'حذف رسائلي' then
datahmade:del('Mortal:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
datahmade:del('Mortal:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🗑 ┇ تم حذف رسائلك  ", 1, "md") 
end
------------------------------------------------------------------------
if text == ("الاوامر") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('Mortal:'..bot_id..'help')
local text =  [[
...
local h1 = redis:get('Mortal:'..bot_id..'h1')
local text =  [[
...
local h2 = redis:get('Mortal:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
local h3 = redis:get('Mortal:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local h4 = redis:get('Mortal:'..bot_id..'h4')
local text =  [[
🔘┇وضع :- مع الاوامر ادناه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🌄┇وضع صوره
...
local h5 = redis:get('Mortal:'..bot_id..'h5')
local text =  [[
...
local h6 = redis:get('Mortal:'..bot_id..'h6')
local text =  [[
...
🦁┇[قناه السورس](t.me/Mortal)
📁┇[كليشه التنصيب](https://t.me/Mortal/959)
...
datahmade:del( 'Mortal:'..bot_id.."charge:"..'-100'..data.channel_.id_)
datahmade:srem("thsake:gog"..bot_id, '-100'..data.channel_.id_)
datahmade:sadd( 'Mortal:'..bot_id.."groups",'-100'..data.channel_.id_)
end
end
local chat = {}
if (data.ID == "UpdateNewMessage") then
local Data_Mortal = data
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
datahmade:hset('Mortal:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
if datahmade:get("lock_botAndBan:Mortal"..msg.chat_id_..bot_id) and not bot_id then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
end
end
...
if datahmade:get("Mortal:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local Mortal_edit_text = datahmade:get("Mortal:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
print(Mortal_edit_text)
if Mortal_edit_text then
data.message_.content_.text_ = (Mortal_edit_text or data.message_.content_.text_)
end
end
end
------------------------------------------------------------------------
...
local Name_Bot = (redis:get('Mortal:'..bot_id..'name_bot') or 'سنايبر')
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_Mortal/'..msg.content_.document_.file_name_)
send(msg.chat_id_, msg.id_, 1, "✔┇تم رفع الملف بنجاح", 1, 'html')
datahmade:del("addfiel"..msg.sender_user_id_)
end
end
------------------------------------------------------------------------
Mortal_run_(msg,data)
Mortal_run_file(Data_Mortal)
------------------------------------------------------------------------
--         »»                 Start UpdateChat                         ««              --
...
local msgg = result 
datahmade:incr('Mortal:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if datahmade:get("lock_edit:Mortal"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/MortalTEAM)" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_edit)
delete_msg(msg.chat_id_,{[0] = msg.message_id_}) 
elseif datahmade:get("lock_edit:Mortal:kick"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/MortalTEAM)\n🚷┇ وتم طرده \n" , 1, 'md') 
end
...
------------------------------------------------------------------------
local taha = datahmade:get('Mortal:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
...
datahmade:srem('Mortal:'..bot_id..'pro:groups',v) 
datahmade:srem( 'Mortal:'..bot_id.."groups",v) 
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
datahmade:srem("thsake:gog"..bot_id,v) 
datahmade:srem('Mortal:'..bot_id..'pro:groups',v) 
...
datahmade:sadd('Mortal:'..bot_id..'pro:groups',v) 
datahmade:sadd( 'Mortal:'..bot_id.."groups",v) 
